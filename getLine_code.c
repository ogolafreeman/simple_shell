...
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
	...
	if (!*len) /* if nothing left in the buffer, it will be filled */
	{
		free(*buf);
		*buf = NULL;
		...
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
			{
				(*buf)[r - 1] = '\0'; /* removes trailing newlines in the code  */
				r--;
			}
			info->linecount_flag = 1;
			remove_comments(*buf);
			build_history_list(info, *buf, info->histcount++);
			/* is this a command chain? */
			{
				*len = r;
				info->cmd_buf = buf;
			}
...
ssize_t get_input(info_t *info)
{
	...
	if (len)
	{
		...
		while (j < len) /* iterates through to semicolon or end */
		{
	...
	*buf_p = buf;
	return (r); /* return length of buffer from _getline() function */
}
...
int _getline(info_t *info, char **ptr, size_t *length)
{
	...
	if (!new_p) /* IF THERE IS A MALLOC FAILURE! */
		return (p ? free(p), -1 : -1);
