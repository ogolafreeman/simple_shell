...
int write_history(info_t *info)
{
	...
	fld = open(filename, O_CREAT | O_TRUNC | O_RDWR, 0644);
	free(filename);
	if (fld == -1)
		return (-1);
	for (node = info->history; node; node = node->next)
	{
		_putsfd(node->str, fld);
		_putfd('\n', fld);
	}
	_putfd(BUF_FLUSH, fld);
	close(fld);
	return (1);
}
...
/**
 * read_history - ths function reads history from file
 * @info: the parameter struct
 *
